...
const Skills: React.FC<AccountProps> = ({ user, isYourAccount }) => {
  ...
  const addSkillHandler = () => {
    //vi ko co gia tri se la false
    //! nguoc lai la true se kich hoat dieu kien
    //Cach 2: if (skill.trim() === "") cung y chang
    if (!skill.trim()) {
      ...
      <Card className="shadow-lg border-2 overflow-hidden">
        ...
        {isYourAccount && (
          <div className="flex gap-3 flex-col sm:flex-row">
            ...
            >
              <Plus size={18} /> {btnLoading ? "Adding..." : "Add Skill"}
            </Button>
          </div>
        )}
            ...
            >
              {user.skills.map((e, i) => (
                <div
                  ...
                  gap-2 border-2 rounded-full hover:shadow-sm 
                  duration-200 transition-all pl-4 pr-3 py-2  "
                  key={i}
                >
                  ...
                  {
                    isYourAccount && (
                      <button onClick={() => removeSkillHandler(e)}
                      className="h-6 w-6 rounded-full text-red-500 flex
                      items-center justify-evenly transition-all hover:bg-gray-600
            ...
            <>
              <div className="text-center py-12">
                ...
                <CardDescription className="text-base">
                  {isYourAccount ? "No Skill Added Yet. Start Building Your Profile!"
